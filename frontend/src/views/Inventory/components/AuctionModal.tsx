...
export default function AuctionModal({ type, blook, item, shiny }: AuctionModalProps) {
    const { user } = useUser();
    if (!user) return null;
    ...
    const submit = () => {
        ...
        if (parseInt(price) < 1) return setError("Price must be at least 1 diamond.");

        if ((auctionTax + durationTax) > user.diamonds) return setError("You don't have enough diamonds to auction this.");

        setLoading(true);
        createAuction({
            type,
            itemId: item ? item.id : undefined,
            blookId: selected ?? undefined,
    ...
    useEffect(() => {
        getRecentAveragePrice({
            ...
            .then((res) => {
                if (res.data.averagePrice !== null) setPrice(Math.floor(res.data.averagePrice * 0.99).toString());
    ...
    useEffect(() => {
        if (!(type === AuctionTypeEnum.BLOOK && blook)) return;
        ...
        if (userBlooks.length > 0) setSelected(userBlooks[0].id);
    }, [type, blook, shiny, user.blooks]);
    ...
    return (
        <>
            <div className={styles.sellBlooksFlex}>
                    ...
                    <div className={styles.sellBlooks}>
                        {user.blooks
                            ...
                            .map((userBlook) => <div
                                ...
                                data-selected={selected === userBlook.id ? "true" : "false"}
                                onClick={() => {
                                    if (selected === userBlook.id) setSelected(null);
                                    else setSelected(userBlook.id);
                                }}
                            >
                ...
                <div className={styles.sellBlooksQuantityContainer}>
