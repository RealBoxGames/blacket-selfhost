...
export class BlooksService {
    ...
    async sellBlooks(userId: string, dto: BlooksSellBlookDto): Promise<void> {
        return await this.prismaService.$transaction(async (tx) => {
            ...
            await tx.userBlook.updateMany({
                where: { id: { in: userBlooks.map((blook) => blook.id) } },
                data: { sold: true }
            });
            await tx.user.update({
                ...
                data: {
                    diamonds: {
                        increment: userBlooks.reduce((acc, blook) =>
                                acc +
                                (blook.shiny? blook.blook.price * 10: blook.blook.price), 0,)
                    }
                }
            });
        });
