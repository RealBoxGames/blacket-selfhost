...
@Throttle({ global: { limit: 1, ttl: seconds(2) } })
export class CosmeticsController {
    constructor(private cosmeticsService: CosmeticsService) {}

    @Patch("avatar")
    @HttpCode(HttpStatus.NO_CONTENT)
    changeAvatar(@GetCurrentUser() userId: string,
        @Body() dto: CosmeticsChangeAvatarDto,) {
    ...
    uploadAvatar(@GetCurrentUser() userId: string,
    ...
    uploadBanner(@GetCurrentUser() userId: string,
    ...
    @Patch("title")
    @HttpCode(HttpStatus.NO_CONTENT)
    changeTitle(@GetCurrentUser() userId: string,
        @Body() dto: CosmeticsChangeTitleDto,) {
    ...
    @HttpCode(HttpStatus.NO_CONTENT)
    changeColorTier1(@GetCurrentUser() userId: string,
        @Body() dto: CosmeticsChangeColorTier1Dto,) {
    ...
    @HttpCode(HttpStatus.NO_CONTENT)
    changeColorTier2(@GetCurrentUser() userId: string,
        @Body() dto: CosmeticsChangeColorTier2Dto,) {
        return this.cosmeticsService.changeColorTier2(userId, dto);
    }

    @Patch("font")
    @HttpCode(HttpStatus.NO_CONTENT)
    changeFont(@GetCurrentUser() userId: string,
        @Body() dto: CosmeticsChangeFontDto,) {
