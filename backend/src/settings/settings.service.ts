...
import {
    SettingsChangeSettingDto,
    SettingsChangeUsernameDto,
    SettingsChangePasswordDto,
    SettingsChangeChatColorDto,
    BadRequest,
    NotFound,
    AuthAuthEntity,
    SettingsEnableOtpDto,
    SettingsDisableOtpDto
} from "@blacket/types";
...
export class SettingsService {
    ...
    async changeSetting(userId: string,
        dto: SettingsChangeSettingDto,): Promise<void> {
        ...
        await this.prismaService.userSetting
            .update({
                where: { id: userId },
                data: { [dto.key]: dto.value === null ? null : dto.value }
            })
            .catch(() => {
                throw new BadRequestException(BadRequest.SETTINGS_INVALID_SETTING_VALUE,);
            });
    ...
    async changeUsername(userId: string,
        dto: SettingsChangeUsernameDto,): Promise<void> {
    ...
    async changePassword(userId: string,
        dto: SettingsChangePasswordDto,): Promise<AuthAuthEntity> {
        const user = await this.usersService.getUser(userId);
        if (!user) throw new NotFoundException(NotFound.UNKNOWN_USER);
        ...
        return {
            token: await this.authService.sessionToToken(await this.authService.findOrCreateSession(userId),)
        } as AuthAuthEntity;
    }

    async changeChatColor(userId: string,
        dto: SettingsChangeChatColorDto,): Promise<void> {
        await this.prismaService.userSetting.update({
            where: { id: userId },
            data: { chatColor: dto.color }
        });
    }
    ...
    async enableOtp(userId: string, dto: SettingsEnableOtpDto): Promise<void> {
        ...
        await this.prismaService.userSetting.update({
            data: { otpSecret: tempOtp },
            where: { id: userId }
        });
    }

    async disableOtp(userId: string,
        dto: SettingsDisableOtpDto,): Promise<void> {
        const user = await this.prismaService.user.findUnique({
            where: { id: userId },
            include: { settings: true }
        });
