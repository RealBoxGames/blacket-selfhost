...
import {
    AuctionsBidAuctionDto,
    AuctionsBuyAuctionDto,
    AuctionsCreateAuctionDto,
    AuctionsSearchAuctionDto,
    BadRequest,
    Forbidden,
    NotFound,
    PrivateUser,
...
export class AuctionsService {
    ...
    async getAuctions(dto: AuctionsSearchAuctionDto = {}): Promise<Auction[]> {
        ...
        const auctions = await this.prismaService.auction.findMany({
            ...
            where: dto.id ? {
                ...
                blook: {
                    blookId: dto.blookId ?? undefined,
                    shiny: dto.shiny ?? undefined
                },
        ...
        for (const auction of auctions) auction.bids = auction.bids.sort((a, b) => b.amount - a.amount);
    ...
    async createAuction(userId: string, dto: AuctionsCreateAuctionDto) {
        ...
        if (
            this.permissionsService.hasPermission(new PrivateUser(user).permissions, PermissionType.LESS_AUCTION_TAX,)
        ) auctionTax = auctionTax * 0.75;
        ...
        return await this.prismaService.$transaction(async (prisma) => {
            const user = await prisma.user.update({
                ...
                data: {
                    diamonds: {
                        decrement: Math.floor(auctionTax) + durationTax
                    }
                }
            });
    ...
    async buyItNow(
        ...
        const event = await this.prismaService.$transaction(async (tx) => {
            ...
            if (claimedAuction.count !== 1) throw new NotFoundException(NotFound.UNKNOWN_AUCTION);

            await tx.user.update({
                where: { id: auction.sellerId },
                data: { diamonds: { increment: auction.price } }
            });

            if (auction.type === AuctionType.BLOOK) await tx.userBlook.update({
                where: { id: auction.blookId },
                data: { user: { connect: { id: userId } } }
            });
            else await tx.userItem.update({
                where: { id: auction.itemId },
                data: { user: { connect: { id: userId } } }
            });

            return new SocketAuctionExpireEntity({
                id: auction.id,
                type: auction.type,
                blook: auction?.blook,
                item: auction?.item,
        ...
        this.socketService.emitAuctionExpireEvent(event);
    }

    async bid(userId: string,
        id: number,
        dto: AuctionsBidAuctionDto,
        ip: string,) {
        const captcha = await this.coreService.verifyTurnstile(dto.captchaToken, ip,);
        if (!captcha) throw new ForbiddenException(Forbidden.DEFAULT);
        ...
        const highestPreviousBidAmount =
            userPreviousBids.length > 0 ? Math.max(...userPreviousBids.map((bid) => bid.amount)) : 0;

        if (dto.amount <= highestPreviousBidAmount) throw new ForbiddenException(Forbidden.AUCTIONS_BID_TOO_LOW.replace("%s", (highestPreviousBidAmount + 1).toLocaleString(),),);
        ...
        await this.prismaService.$transaction(async (tx) => {
            ...
            this.socketService.emitAuctionBidEvent(new SocketAuctionBidEntity({
                ...
                bidders: [
                    ...new Set(auction.bids.map((bid) => bid.user.id))
                ]
            }),);
        });
    }
    ...
    //         await this.redisService.setKey("recentAveragePrice", dto, {
    //             averagePrice: null,
    //             lowestPrice: null,
    //             highestPrice: null,
    //             suspicious: false
    ...
    //         auction.buyItNow ? auction.price : auction.bids.sort((a, b) => b.amount - a.amount)[0].amount,);
    //     prices.sort((a, b) => a - b);
