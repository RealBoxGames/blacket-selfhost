...
export class CronService {
    constructor(private loggerService: BlacketLoggerService,
    ...
    async checkAuctions() {
        ...
        for (const auction of expiredAuctions) {
            ...
            auction.bids = auction.bids.sort((a, b) => b.amount - a.amount);
        }
        ...
        await this.prismaService.$transaction(async (tx) => {
            for (const auction of expiredAuctions) {
                ...
                await tx.user.update({
                    where: { id: auction.seller.id },
                    data: { diamonds: { increment: winningBid.amount } }
                });

                switch (auction.type) {
                    case AuctionType.BLOOK:
                        await tx.userBlook.update({
                            where: { id: auction.blook.id },
                            data: { userId: winningBid.user.id }
                        });
                        break;
                ...
                const losingBidders = auction.bids.reduce((acc, bid) => {
                    if (
                        !acc[bid.user.id] ||
                        acc[bid.user.id].amount < bid.amount
                    ) {
                        acc[bid.user.id] = bid;
                    }
