...
export class S3Controller {
    ...
    @Get("upload/small")
    async uploadFileSmall(@GetCurrentUser() userId: string,
        @Query() dto: S3UploadDto,): Promise<S3UploadEntity> {
        return await this.s3Service.createPresignedPost(userId, dto.filename, dto.mimetype, 1024 * 1024 * 10,);
    }

    @Permissions({ permissions: [PermissionType.UPLOAD_FILES_LARGE] })
    @Throttle({ global: { limit: 100, ttl: hours(1) } })
    @Get("upload/large")
    async uploadFileLarge(@GetCurrentUser() userId: string,
        @Query() dto: S3UploadDto,): Promise<S3UploadEntity> {
        return await this.s3Service.createPresignedPost(userId, dto.filename, dto.mimetype, 1024 * 1024 * 100,);
    }

    @Throttle({ global: { limit: 100, ttl: hours(1) } })
    @Post("verify")
    async verifyFile(@GetCurrentUser() userId: string,
        @Body() dto: S3VerifyDto,): Promise<Upload> {
