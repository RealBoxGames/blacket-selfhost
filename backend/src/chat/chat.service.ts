...
export class ChatService {
    ...
    async createMessage(userId: string,
        roomId: number,
        dto: ChatCreateMessageDto,): Promise<Message> {
        const room = await this.redisService.getRoom(roomId);
        if (!room) throw new NotFoundException(NotFound.UNKNOWN_ROOM);
        ...
        if (!room.public && !room.users.find((user) => user.id === userId)) throw new ForbiddenException(Forbidden.CHAT_ROOM_NO_PERMISSION);
            ...
            if (mute) throw new ForbiddenException(Forbidden.CHAT_MUTED.replace("%s", mute.reason).replace("%s", `${
                            mute.expiresAt.getTime() - Date.now() >
                            1000 * 60 * 60 * 24 * 365? "never": `on ${mute.expiresAt.toLocaleDateString()} at ${mute.expiresAt.toLocaleTimeString()} UTC`
                        }`,),);
        }
    ...
    async deleteMessage(userId: string,
        ...
        if (room.public && message.authorId !== userId && !isStaffMember) throw new ForbiddenException(Forbidden.CHAT_MESSAGE_NO_PERMISSION);
        else if (message.authorId !== userId) throw new ForbiddenException(Forbidden.CHAT_MESSAGE_NO_PERMISSION);
    ...
    async editMessage(userId: string,
        roomId: number,
        messageId: string,
        dto: ChatEditMessageDto,): Promise<void> {
        const room = await this.redisService.getRoom(roomId);
        if (!room) throw new NotFoundException(NotFound.UNKNOWN_ROOM);
